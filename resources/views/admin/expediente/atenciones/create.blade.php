<x-admin-layout title="Registrar Atención" :breadcrumbs="[
    ['name' => 'Dashboard', 'href' => route('admin.dashboard')],
    ['name' => 'Citas', 'href' => route('admin.appointments.index')],
    ['name' => 'Cita #' . $appointment->id, 'href' => route('admin.appointments.show', $appointment)],
    ['name' => 'Registrar Atención'],
]">
    <form action="{{ route('admin.appointments.atencion.store', $appointment) }}" method="POST">
        @csrf
        ...
        <div class="grid lg:grid-cols-2 gap-6">
            ...
            <div class="space-y-6">
                ...
                <x-wire-card>
                    ...
                    <x-wire-textarea label="" name="sintomatologia" rows="4" placeholder="Describa los síntomas referidos por el paciente...">{{ old('sintomatologia') }}</x-wire-textarea>
                </x-wire-card>
                ...
                <x-wire-card>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <x-wire-input label="I.M.C." name="imc" value="{{ old('imc') }}" placeholder="ej. 16.3" />
                    </div>
                </x-wire-card>
                ...
                <x-wire-card>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <x-wire-input label="Presión Arterial" name="presion_arterial" value="{{ old('presion_arterial') }}" placeholder="ej. 100/60 mmHg" />
                    </div>
                </x-wire-card>
            ...
            <div class="space-y-6">
                ...
                {{-- Estudios Ordenados --}}
                <x-wire-card x-data="{
                    estudios: {{ old('estudios') ? json_encode(old('estudios')) : '[]' }},
                    agregar() {
                ...
                <x-wire-card>
                    ...
                    <div class="space-y-4">
                        <x-wire-textarea label="Diagnóstico Posible" name="diagnostico_posible" rows="3">{{ old('diagnostico_posible') }}</x-wire-textarea>
                        <x-wire-textarea label="Diagnóstico Confirmado" name="diagnostico_confirmado" rows="3">{{ old('diagnostico_confirmado') }}</x-wire-textarea>
                    </div>
                </x-wire-card>
                ...
                <x-wire-card>
                    ...
                    <x-wire-textarea label="" name="medicacion_indicada" rows="4" placeholder="Liste los medicamentos indicados con dosis y frecuencia...">{{ old('medicacion_indicada') }}</x-wire-textarea>
                </x-wire-card>

                {{-- Notas internas --}}
                <x-wire-card>
                    <h3 class="font-semibold text-gray-700 mb-3">Notas</h3>
                    <x-wire-textarea label="" name="notas" rows="3" placeholder="Observaciones adicionales...">{{ old('notas') }}</x-wire-textarea>
                </x-wire-card>
